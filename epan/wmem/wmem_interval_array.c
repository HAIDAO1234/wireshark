...
wmem_iarray_t *
wmem_iarray_new(wmem_allocator_t *scope, const guint elem_size, GEqualFunc equal)
{
	wmem_iarray_t *iarr;

	if (elem_size < sizeof(range_t)) return NULL;

	iarr = wmem_new(scope, wmem_iarray_t);
	if (!iarr) return NULL;
...
gboolean
_wmem_iarray_is_empty(wmem_iarray_t *iarr)
{
...
range_admin_t *
wmem_iarray_find(wmem_iarray_t *iarr, guint32 value) {
	wmem_iarray_sort(iarr);
...
void
wmem_print_iarr(wmem_iarray_t *iarr)
{
	range_admin_t *elem;
	guint i, len;
