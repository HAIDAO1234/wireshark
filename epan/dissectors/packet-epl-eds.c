...
#include <epan/wmem/wmem_interval_array.h>
...
{
	wmem_iarray_t *subindices = ((struct object*)value)->subindices;
	if (subindices)
		wmem_iarray_sort(subindices);
}
...
eds_load(struct profile *profile, const char *eds_file)
{
	...
	for (group = groups; *group; group++)
	{
		...
		{ /* Object already there, let's add subindices */
			...
			if (!obj->subindices)
			{
				obj->subindices = wmem_iarray_new(
						profile->scope,
			...
			subobj.info = tmpobj;
			wmem_iarray_insert(obj->subindices, subobj.info.idx, &subobj.range);
		}
	}
